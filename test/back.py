...
def replace_in_csv(csv_file, column_to_replace):
    ...
    sort_column = 'TagID'
    # Sort both files based on TagID	OperationType	TagComments	DependencyValue
    ...
    df=file1_sorted
    df['TagID'] = df['TagID'].apply(lambda x: x.replace('yy', '') if 'yy' in x else x)
